...
dat = dat %>%
  ...
  ungroup()
# matches work done in Stata (takes ~30s as groups by personID)
```
...
cleaned = dat %>% subset(
  ...
  mutate(
    ...
    age_psid = relevel(factor(age_psid_imputed), ref = "40"), # imputes to not shed rows
...
plot_data = data.frame(
  year = c(event_years, 0), # Add the reference year (0)
  estimate = c(event_coefs[, 1], 0), # Add 0 for the reference group's coefficient
  se = c(event_coefs[, 2], 0), # Add 0 for the reference group's SE
